...
def _account_card(account, net_worth_previous_cad):
    ...
    return f'''<article class="account" style="--accent:{account["accent"]}">
      <div class="account-head"><div><div class="account-kicker">{escape(account["label"])}</div><div class="account-value">{escape(account["value_label"])}</div></div><div class="source">{escape(account.get("source", ""))}</div></div>
