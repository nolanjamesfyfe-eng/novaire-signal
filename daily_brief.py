...
def _account_card(account, net_worth_previous_cad):
    ...
    return f'''<details class="account" style="--accent:{account["accent"]}">
      ...
      <div class="position"><div class="position-name"><small>LARGEST POSITION</small><div class="symbol">{escape(account["symbol"])}</div><div class="lead">{account.get("weight_pct", 0):.1f}% of portfolio</div></div><div class="position-move"><small>DAILY POSITION MOVE</small><div class="daily {cls}">{_pct(daily)}</div></div></div>
      <div class="quote-grid"><div><small>DAY HIGH</small><b>{high}</b></div><div><small>DAY LOW</small><b>{low}</b></div></div>
      <div class="impact-grid"><div><small>PORTFOLIO DAILY EFFECT</small><b class="{cls}">{_money(impact_cad, "C$", 0)}</b><span class="metric-sub {cls}">{_pct(portfolio_impact, 2)} of portfolio</span></div><div><small>INTRADAY CASH SWING</small><b>{range_money}</b><span class="metric-sub">high to low</span></div><div><small>NET WORTH EFFECT</small><b class="{cls}">{_pct(net_impact, 2)}</b><span class="metric-sub">of tracked net worth</span></div></div>{mtd_ytd}
      </div>
    </details>'''
