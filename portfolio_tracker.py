...
def _interactive_chart_html(model: dict[str, Any]) -> str:
    ...
    return f'''<div class="tracker-hero" data-series="{payload}" data-ath-cad="{ath_cad:.2f}" data-ath-source="{ath_source}">
      ...
      <svg class="tracker-hero-svg" viewBox="0 0 920 330" preserveAspectRatio="none" role="img" aria-label="Interactive total net worth history">
        ...
        <path class="tracker-hero-area" fill="url(#netWorthFill)"/><path class="tracker-hero-line" fill="none" stroke="#56f2b1" stroke-width="3.5" vector-effect="non-scaling-stroke" stroke-linecap="round" stroke-linejoin="round"/><line class="tracker-crosshair" y1="16" y2="308"/><ellipse class="tracker-dot" vector-effect="non-scaling-stroke"/>
    ...
    <script>(function(){{
      ...
      svg.addEventListener('pointermove',e=>{{const r=svg.getBoundingClientRect(),x=(e.clientX-r.left)/r.width*W,i=Math.max(0,Math.min(shown.length-1,Math.round((x-P)/(W-2*P)*(shown.length-1)))),p=shown[i],vals=shown.map(q=>q.cad),lo=Math.min(...vals),hi=Math.max(...vals),pad=Math.max((hi-lo)*.13,1),cx=P+(W-2*P)*(i/Math.max(shown.length-1,1)),cy=P+(H-2*P)*(1-(p.cad-(lo-pad))/((hi+pad)-(lo-pad)));dot.setAttribute('cx',cx);dot.setAttribute('cy',cy);dot.setAttribute('rx',6*W/r.width);dot.setAttribute('ry',6*H/r.height);cross.setAttribute('x1',cx);cross.setAttribute('x2',cx);dot.style.opacity=cross.style.opacity=1;root.querySelector('.tracker-hero-value').textContent='C$'+p.cad.toLocaleString('en-CA',{{minimumFractionDigits:2,maximumFractionDigits:2}});showAth(p);note.textContent=new Date(p.market_date+'T00:00:00Z').toLocaleDateString('en-CA',{{month:'long',day:'numeric',year:'numeric'}})}});svg.addEventListener('pointerleave',()=>{{root.querySelector('.tracker-hero-value').textContent='C$'+all.at(-1).cad.toLocaleString('en-CA',{{minimumFractionDigits:2,maximumFractionDigits:2}});draw(root.querySelector('.tracker-range.is-active').dataset.range)}});draw('YTD');
    }})();</script>'''
