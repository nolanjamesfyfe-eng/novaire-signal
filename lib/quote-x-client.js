...
function createXClient({fetchImpl=global.fetch,credentials,timeoutMs=8000}){
  ...
  async function findRecent(text,userId){const result=await request('GET',`/2/users/${enc(userId)}/tweets?max_results=5&${fields}`);const cutoff=Date.now()-10*60*1000;return(result.data||[]).find(p=>p&&verifiedText(p,String(p.id))===text&&(!p.created_at||Date.parse(p.created_at)>=cutoff));}
  async function publish({text,png}){
    ...
    let existing=await findRecent(text,userId);if(existing){const out=await verifyPost(String(existing.id),text,userId);return{...out,replayed:true};}
    ...
    let created;try{created=await request('POST','/2/tweets',{text,media:{media_ids:[mediaId]}});}catch(error){
      ...
      try{existing=await findRecent(text,userId);if(existing){const out=await verifyPost(String(existing.id),text,userId);return{...out,replayed:true};}}catch{}
