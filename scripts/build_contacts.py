...
html = f'''<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><meta name="theme-color" content="#090909"><title>Connect · Nolan James Fyfe</title><link rel="icon" href="/icon-192.png"><link rel="manifest" href="/manifest.webmanifest"><link rel="apple-touch-icon" href="/icon-192.png"><meta name="apple-mobile-web-app-capable" content="yes"><meta name="apple-mobile-web-app-title" content="My Contacts"><meta name="apple-mobile-web-app-status-bar-style" content="black"><script>if('serviceWorker' in navigator){{window.addEventListener('load',()=>navigator.serviceWorker.register('/sw.js',{{scope:'/'}}).catch(console.error))}}</script><style>
...
</style></head><body><main><header><p class="eyebrow">Let's connect</p><h1>Nolan James Fyfe</h1><p class="intro">Scan a code, or tap to open.</p><button id="install" type="button" hidden>Install Contacts App</button></header><section class="cards" aria-label="Social profiles">{''.join(cards)}</section><footer><a href="https://novairesignal.com/">NOVAIRE SIGNAL</a></footer></main><script>
let installPrompt;
...
manifest = {
    "id": "/",
    "name": "My Contacts · Novaire",
    "short_name": "My Contacts",
...
assets = ["/", "/index.html", "/linkedin.png", "/instagram.png", "/whatsapp.png", "/manifest.webmanifest", "/icon-192.png", "/icon-512.png"]
(OUT / "sw.js").write_text(f"""const CACHE='novaire-contacts-origin-v1';
