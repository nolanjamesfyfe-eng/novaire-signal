...
class RenderContractTests(unittest.TestCase):
    ...
    def test_portfolio_tickers_open_nine_month_weekly_candles(self):
        ...
        self.assertIn("CLOSE ${fmt(last.close)}", self.portfolio_html)
    ...
    def test_clutter_cards_are_accordions_with_compact_summaries(self):
        ...
        self.assertIn("const fingerprint = card.dataset.fingerprint", self.html)
        self.assertIn("if (localStorage.getItem(key) === fingerprint) card.removeAttribute('open')", self.html)
        self.assertIn("else localStorage.setItem(key, fingerprint)", self.html)
